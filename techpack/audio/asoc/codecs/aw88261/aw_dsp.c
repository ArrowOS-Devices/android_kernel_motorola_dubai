...
extern int aw_send_afe_tx_module_enable(int32_t tx_port_id, void *buf, int size);
extern int aw_adm_param_enable(int port_id, int module_id, int param_id, int enable);
#else
static int aw_send_afe_cal_apr(uint32_t rx_port_id, uint32_t tx_port_id, uint32_t param_id,
	void *buf, int cmd_size, bool write)
{
	return 0;
}
static int aw_send_afe_rx_module_enable(int32_t rx_port_id, void *buf, int size)
{
	return 0;
}
static int aw_send_afe_tx_module_enable(int32_t tx_port_id, void *buf, int size)
{
	return 0;
}
				...
				uint32_t *enable, uint32_t *relase_time)
{
	...
	*relase_time = spin_param.relase_time;
	...
	aw_pr_info("get spin params done");
	return 0;
}
				...
				uint32_t enable, uint32_t relase_time)
{
	...
	if (!enable) {
		aw_pr_dbg("need to set the rotation parameters");
		return 0;
	}
